#persistantViperClient  to exe run commands below
...
cd payloads/
wine pyinstaller --onefile --noconsole --nowindowed persistantViperClient.py
rm -rf build/
rm -rf dist/
sed -i 's/console=True/console=False/g' persistantViperClient.spec
wine pyinstaller persistantViperClient.spec
mv dist/persistantViperClient.exe .
rm -rf build/
rm -rf dist/
cd ../